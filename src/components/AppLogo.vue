...
.app-logo-container {
  ...
  & .logo-bg.visible-bg { // .visible-bg 类由 <template> 中的 :class 绑定
    opacity: 1;
    transform: scale(1); // 恢复到正常大小
  }
  // 鼠标悬停且抽屉关闭时的额外动画 (可选)
  & .logo-bg.animate-bg { // 鼠标悬停在关闭的抽屉按钮上时，背景的动画
    // transform: scale(1.1) rotate(10deg); // 可以是不同的动画
  }
  & .logo-icon.animate-icon { // 鼠标悬停在关闭的抽屉按钮上时，前景的动画
